...
class EditOrder extends Component{
    ...
    submit = (e) =>{
        ...
        service.api(ORDER_URI_ONE+"/"+this.props.match.params.id,{
            body:JSON.stringify({
    ...
    render(){
        ...
        if(this.props.menu.menu.length !== 0){
             meals = this.props.menu.menu.menu_items.map(meal=>(
                <option value={meal.meal.id} key={meal.id}>{meal.meal.name}</option>  
            ))
        }
                ...
                <form onSubmit={this.submit}>
                    <div className="form-group">
                        <label htmlFor="Meal">Meal</label>
                            <select name="meal" id="" className="form-control" onChange={this.handleChange}>
                                {meals ? meals : ""}
                            </select>
                    </div>
